...
void Matrix::add(const Matrix &other, REAL s){
  CHECK_DEBUG4(m == other.m && n == other.n, "Array Size (%d, %d) += (%d, %d)", m, n, other.m, other.n);
...
void Matrix::add(const Matrix &m1, const Matrix &m2){
  CHECK_DEBUG4(m1.m == m2.m && m1.n == m2.n, "Array Size (%d, %d) + (%d, %d)", m1.m, m1.n, m2.m, m2.n);
  CHECK_DEBUG4(allocM >= m1.m && allocN >= m1.n, "Array Size (%d, %d) <- (%d, %d)", allocM, allocN, m1.m, m1.n);
...
void Matrix::mult(const Matrix &m1, const Matrix &m2){
  CHECK_DEBUG4(m1.n == m2.m, "Matrix Size (%d, %d) * (%d, %d)", m1.m, m1.n, m2.m, m2.n);
  CHECK_DEBUG4(allocM >= m1.m && allocN == m2.n, "Matrix Size (%d, %d) <- (%d, %d)", allocM, allocN, m1.m, m1.n);
...
bool Matrix::invert(const Matrix &other){
  CHECK_DEBUG2(other.m == other.n && other.m >= 2, "Invalid Matrix (%d, %d)", other.m, other.n);
  CHECK_DEBUG4(allocM >= other.m && allocN >= other.n, "Array Size (%d, %d) <- (%d, %d)", allocM, allocN, other.m, other.n);
  ...
  if (other.m == 2){   //  special efficient case
    REAL det = other.index(0,0)*other.index(1,1) - other.index(0,1)*other.index(1,0);
