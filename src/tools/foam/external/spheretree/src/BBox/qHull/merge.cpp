...
void qh_flippedmerges(facetT *facetlist, boolT *wasmerge) {
  ...
  FOREACHmerge_(othermerges) {
    ...
    if (merge->type != MRGflip || facet1->visible) 
      continue;
    if (qh OUTPUTINFOmerge-1 == zzval_(Ztotmerge))
      qhmem.IStracing= qh IStracing= qh OUTPUTINFOlevel;
...
void qh_mergecycle (facetT *samecycle, facetT *newfacet) {
  ...
  if (qh POSTmerging && zzval_(Ztotmerge) > qh mergereport + qh REPORTfreq2)
    qh_tracemerging();
#ifndef qh_NOtrace
  if (qh OUTPUTINFOmerge == zzval_(Ztotmerge))
    qhmem.IStracing= qh IStracing= qh OUTPUTINFOlevel;
...
-mergefacet- merges facet1 into facet2
  traces merge if fmax_(maxdist,-mindist) > OUTPUTINFOdist
...
void qh_mergefacet(facetT *facet1, facetT *facet2, realT *mindist, realT *maxdist, boolT mergeapex) {
  ...
  if (qh POSTmerging && zzval_(Ztotmerge) > qh mergereport + qh REPORTfreq2)
    qh_tracemerging();
#ifndef qh_NOtrace 
  if (mindist && (-*mindist > qh OUTPUTINFOdist || *maxdist > qh OUTPUTINFOdist)) {
    tracerestore= 0;
...
void qh_mergevertex_neighbors(facetT *facet1, facetT *facet2) {
  ...
  if (qh tracevertex) {
    ...
    qh_errprint ("OUTPUTINFO", NULL, NULL, NULL, qh tracevertex);
  }
  FOREACHvertex_(facet1->vertices) {
    if (vertex->visitid != qh vertex_visit) 
      qh_setreplace(vertex->neighbors, facet1, facet2);
...
void qh_tracemerge (facetT *facet1, facetT *facet2) {
  ...
  if (facet2 == qh tracefacet || (qh tracevertex && qh tracevertex->newlist)) {
    ...
    if (facet2 != qh tracefacet)
      qh_errprint ("OUTPUTINFO", qh tracefacet, 
