...
facetT *qh_findbestnew (pointT *point, facetT *startfacet,
  ...
  if (qh OUTPUTINFOpoint >= 0 && qh OUTPUTINFOpoint == qh_pointid (point)) {
    ...
    fprintf(qh ferr, "qh_findbestnew: point p%d facet f%d. Stop if dist > %2.2g\n",
	     qh OUTPUTINFOpoint, startfacet->id, distoutside);
    fprintf(qh ferr, "  Last point added to hull was p%d.", qh furthest_id);
    fprintf(qh ferr, "  Last merge was #%d.\n", zzval_(Ztotmerge));
  }
  if (isoutside)
...
void qh_setfacetplane(facetT *facet) {
  ...
  if (qh PRINTstatistics || qh IStracing || qh OUTPUTINFOlevel) {
	...
	zinc_(Zdiststat);
	  ...
	  if (dist > qh max_outside) {
	    qh max_outside= dist;
	    if (dist > qh OUTPUTINFOdist) 
	      istrace= True;
	  }
	}else if (-dist > qh OUTPUTINFOdist)
	  istrace= True;
	if (istrace) {
