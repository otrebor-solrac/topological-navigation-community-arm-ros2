...
inline void GDIAM_COPY(gdiam_point_t &dest, const gdiam_point_t &src){
  dest[0] = src[0];
  dest[1] = src[1];
  dest[2] = src[2];
}

inline void GDIAM_COPY(gdiam_point_2d_t &dest, const gdiam_point_2d_t &src){
  dest[0] = src[0];
  dest[1] = src[1];
}
...
public:
    ...
    void  set_third_dim_longest() {
        ...
        if  ( ( high_1 - low_1 ) > ( high_3 - low_3 ) ) {
            ...
            GDIAM_COPY(pnt_tmp, dir_1);
            GDIAM_COPY(dir_1, dir_3);
            GDIAM_COPY(dir_3, pnt_tmp);
        }
        if  ( ( high_2 - low_2 ) > ( high_3 - low_3 ) ) {
            gdiam_exchange( high_2, high_3 );
            gdiam_exchange( low_2, low_3 );

            GDIAM_COPY(pnt_tmp, dir_2);
            GDIAM_COPY(dir_2, dir_3);
            GDIAM_COPY(dir_3, pnt_tmp);
        }
    }
